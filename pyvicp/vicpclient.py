...
class RecvBlock:
    ...
    read_state: int = READSTATE.header  # current state of read 'state machine'
    flags: int = 0  # flags byte from received header
    block_size: int = 0  # total number of payload bytes
    bytes_read: int = 0  # number of payload bytes received so far
    eoi_terminated: bool = False  # flag indicating whether current block
    srq_state: int = False  # indicates state of SRQ
    srq_state_changed: bool = False  # indicates a change in SRQ state
    seq_num: int = 0  # received sequence number
...
class Client:
    ...
    # --------------------------------------------------------------------------
    def receive(self, buf_len: int | None = None) -> bytes:
        return self._read_till_whenever(buf_len, stop_at_eob=False)

    # --------------------------------------------------------------------------
    def recv_block(self, buf_len: int | None = None) -> bytes:
...
class AsyncClient:
    ...
    def _get_next_seq_num(self, eoi_termination: bool) -> int:
        next(self._seq_coro)
        seq_num = self._seq_coro.send(eoi_termination)
        assert seq_num is not None
        self._last_seq_num = seq_num
        return self._last_seq_num

    async def _send_packet(
        self, payload: bytes = b"", flags: int = OPERATION.eoi
    ) -> None:
        assert self._writer is not None
        eoi = (flags & OPERATION.eoi) != 0
        seq = self._get_next_seq_num(eoi)
        logger.debug(f"_send_packet: {seq=} {eoi=}")

        flags |= OPERATION.data
        payload_length = len(payload)
        msg = (
            struct.pack(HEADER_FORMAT, flags, HEADER_VERSION1, seq, 0, payload_length)
            + payload
        )
    ...
    async def _receive_flush(self, bytes_to_dump: int) -> None:
        ...
        await self._receive_exact(bytes_to_dump)

    async def _receive_exact(self, length: int):
    ...
    async def _read_from_device(
        ...
        while True:
            ...
            if self._recv_block.read_state == READSTATE.data:
                reply_buf += await self._receive_exact(self._recv_block.block_size)
                self._recv_block.read_state = READSTATE.header
                ...
                if len(reply_buf) >= req_len:
                    # the user's buffer has been filled.  get out of the loop.
                    break

        logger.debug(f"_read_from_device: returning {len(reply_buf)} bytes")

        return reply_buf

    async def _read_srq_packet(self) -> None:
        """
        read the SRQ packet, process it, and toss it.
        errors will raise an exception.
        """
        assert self._recv_block.read_state == READSTATE.data
        srq_packet = await self._receive_exact(self._recv_block.block_size)
        self._recv_block.read_state = READSTATE.header
        # '1' = asserted, '0' = deasserted
        self._recv_block.srq_state = srq_packet[0] == "1"

    async def _read_next_header(self) -> None:
        """
        read the next header, flushing any unread (old) responses along the way.
        errors will raise an exception.
        """
        while self._recv_block.read_state == READSTATE.header:
            self._recv_block = await self._get_recv_block()

            # header was successfully read
            logger.debug(
                "Read Header: "
                f"block_size={self._recv_block.block_size}, "
                f"EOI={self._recv_block.eoi_terminated}, "
                f"SRQ state changed={self._recv_block.srq_state_changed}, "
                f"seq_num={self._recv_block.seq_num}"
            )

            if self._recv_block.srq_state_changed:
                await self._read_srq_packet()
                raise SrqStateChanged
            ...
            if self._recv_block.seq_num == 0:
                ...
                # then dump this block and go around again.
                seq_num_delta = (
                    self._last_seq_num - self._recv_block.seq_num + 255
                ) % 255
                if self._flush_unread_responses and seq_num_delta > 0:
                    await self._receive_flush(self._recv_block.block_size)
                    self._recv_block.read_state = READSTATE.header

    async def _get_recv_block(self) -> RecvBlock:
        header_size = struct.calcsize(HEADER_FORMAT)
        header_buf = await self._receive_exact(header_size)
